...
* **База**: миграции `V21__moex_netflow2.sql`/`V22__moex_netflow2_realign.sql` создают `moex_netflow2` с PK(date, ticker), CHECK на непустой ticker, индексом по (ticker, date) и COMMENT. `V22` аккуратно отрабатывает сценарий с устаревшей таблицей (sec → ticker). Upsert через `ON CONFLICT` идемпотентен.
