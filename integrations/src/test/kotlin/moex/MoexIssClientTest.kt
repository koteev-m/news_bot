...
class MoexIssClientTest {
    ...
    @Test
    fun getSecuritiesTqbrSucceedsWithConfiguredClient() = runTest {
        val cfg = TestHttpFixtures.defaultCfg()
        val metrics = TestHttpFixtures.metrics()
        val http = TestHttpFixtures.client(cfg, metrics) {
            addHandler { _ ->
                respond(
                    content = """
                        {
        ...
        assertEquals("SBER", result.getOrThrow().securities.first().code)
