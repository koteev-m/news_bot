...
class ClientsSmokeTest {
    @Test
    fun clientsRetryAndReportMetrics() =
        runTest {
            ...
            val http =
                TestHttpFixtures.client(cfg, metrics, clock) {
                    addHandler { request ->
                        ...
                        if (count == 1) {
                            ...
                            when (path) {
                                "/iss/engines/stock/markets/shares/marketstatus.json" ->
                                    respond(
            ...
            val cbrClient =
                CbrClient(http, cbrCb, metrics, cacheTtlMs = 15_000L, clock = clock).apply {
                    setBaseUrl("https://example.test")
                }
    ...
    fun openBreakerPreventsCall() =
        runTest {
            ...
            val moexClient =
                MoexIssClient(http, cb, metrics, cacheTtlMs = 15_000L, statusCacheTtlMs = 5_000L, clock = clock).apply {
                    setBaseUrl("https://example.test")
                }
