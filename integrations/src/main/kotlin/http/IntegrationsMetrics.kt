...
class IntegrationsMetrics(
    ...
    fun retryCounter(service: String): Counter =
        retryCounters.computeIfAbsent(service) {
            registry.counter("integrations_retry_total", "service", it)
        }

    fun retryAfterHonored(service: String): Counter =
        retryAfterCounters.computeIfAbsent(service) {
            registry.counter("integrations_retry_after_honored_total", "service", it)
        }

    fun cbOpenCounter(service: String): Counter =
        cbOpenCounters.computeIfAbsent(service) {
    ...
    fun requestTimer(
        ...
        requestTimers.computeIfAbsent(service to outcome) {
