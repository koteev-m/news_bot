...
class DefaultRbacService : RbacService {
    override fun canReadPortfolio(ctx: TenantContext): Boolean =
        Role.VIEWER in ctx.roles || canWritePortfolio(ctx)
    ...
    override fun canAdmin(ctx: TenantContext): Boolean =
        Role.ADMIN in ctx.roles || Role.OWNER in ctx.roles
}
