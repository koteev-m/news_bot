...
class CsvImportServiceTest {
    ...
    @Test
    fun `imports sample csv file`() =
        runBlocking {
            val resolver =
                FakeInstrumentResolver().apply {
    ...
    @Test
    fun `skips duplicates by ext id and soft key`() =
        runBlocking {
            val resolver =
                FakeInstrumentResolver().apply {
            ...
            val duplicateKey =
                CsvImportService.SoftTradeKey.of(
    ...
    @Test
    fun `records failures but continues import`() =
        runBlocking {
            val resolver =
                FakeInstrumentResolver().apply {
    ...
    @Test
    fun `resolves instruments through aliases`() =
        runBlocking {
            val resolver =
                FakeInstrumentResolver().apply {
    ...
    private class FakeInstrumentResolver : CsvImportService.InstrumentResolver {
        ...
        override suspend fun findBySymbol(
            ...
            val key =
                SymbolKey(exchange.uppercase(Locale.ROOT), board?.uppercase(Locale.ROOT), symbol.uppercase(Locale.ROOT))
        ...
        fun registerSymbol(
            ...
            bySymbol[
                SymbolKey(
                    exchange.uppercase(Locale.ROOT),
                    board?.uppercase(Locale.ROOT),
    ...
    private class FakeTradeLookup(
        ...
        override suspend fun existsByExternalId(
            portfolioId: UUID,
            externalId: String,
        ): DomainResult<Boolean> = DomainResult.success(existingExtIds.contains(externalId))

        override suspend fun existsBySoftKey(key: CsvImportService.SoftTradeKey): DomainResult<Boolean> =
            DomainResult.success(existingSoftKeys.contains(key))
    }
