...
class PortfolioServiceTest {
    ...
    @Test
    fun `applies buy trade and stores updated average position`() =
        runBlocking {
    ...
    @Test
    fun `rejects sell trade that exceeds current quantity`() =
        runBlocking {
            val fifoCalc = PositionCalc.FifoCalc()
            var position = PositionCalc.Position.empty("USD")
            position =
                fifoCalc
                    .applyBuy(position, BigDecimal("3"), usd("50"), usd("0"))
                    .position
            val stored =
    ...
    @Test
    fun `applies fifo sell and records realized pnl`() =
        runBlocking {
            val fifoCalc = PositionCalc.FifoCalc()
            var position = PositionCalc.Position.empty("USD")
            position =
                fifoCalc
                    .applyBuy(position, BigDecimal("5"), usd("100"), usd("0"))
                    .position
            position =
                fifoCalc
                    .applyBuy(position, BigDecimal("3"), usd("120"), usd("0"))
                    .position
            val stored =
                PortfolioService.StoredPosition(
                    portfolioId,
    ...
    @Test
    fun `rejects trade when currency mismatches stored position`() =
        runBlocking {
    ...
    @Test
    fun `lists open positions with unrealized pnl`() =
        runBlocking {
            val storage =
                FakeStorage(
                    positions =
    ...
    private class FakeStorage(
        ...
        override suspend fun <T> transaction(
            ...
            val tx =
                object : PortfolioService.Storage.Transaction {
                    override suspend fun loadPosition(
                        ...
                        return if (stored != null &&
                            stored.portfolioId == portfolioId &&
                            stored.instrumentId == instrumentId &&
                            stored.valuationMethod == method
                        ) {
    ...
    private class StaticMoexProvider(
        ...
        override suspend fun closePrice(
            instrumentId: Long,
            on: LocalDate,
        ): DomainResult<Money?> = DomainResult.success(closePrices[instrumentId to on])

        override suspend fun lastPrice(
            instrumentId: Long,
            on: LocalDate,
        ): DomainResult<Money?> = DomainResult.success(null)
    }
