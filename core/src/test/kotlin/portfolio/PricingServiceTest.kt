...
class PricingServiceTest {
    ...
    @Test
    fun `returns close price from moex`() =
        runBlocking {
            val moex =
                StubMoexProvider().apply {
    ...
    @Test
    fun `falls back to last price when close missing`() =
        runBlocking {
            val moex =
                StubMoexProvider().apply {
    ...
    @Test
    fun `honors fallback configuration`() =
        runBlocking {
            val moex =
                StubMoexProvider().apply {
                    closeHandler = { _, _ -> DomainResult.success(null) }
    ...
    @Test
    fun `uses close as secondary when last preferred`() =
        runBlocking {
            val moex =
                StubMoexProvider().apply {
                    closeHandler = { _, _ ->
    ...
    @Test
    fun `returns not found when providers lack data`() =
        runBlocking {
    ...
    @Test
    fun `propagates provider failure`() =
        runBlocking {
    ...
    @Test
    fun `propagates fx conversion failure`() =
        runBlocking {
            val moex =
                StubMoexProvider().apply {
    ...
    private class StubMoexProvider :
        StubPriceProvider(),
        MoexPriceProvider

    private class StubCoingeckoProvider :
        StubPriceProvider(),
    ...
    private class FakeFxRateRepository(
        ...
        override suspend fun findOnOrBefore(
            ccy: String,
            timestamp: Instant,
        ): FxRate? = rates[ccy]?.filter { it.ts <= timestamp }?.maxByOrNull { it.ts }
    }
