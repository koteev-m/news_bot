...
class BillingServiceTest {
    ...
    @Test
    fun `listPlans sorts tiers ascending`() =
        runBlocking {
            val repo =
                FakeRepo(fixedNow).apply {
    ...
    @Test
    fun `createInvoiceFor rejects free tier`() =
        runBlocking {
    ...
    @Test
    fun `createInvoiceFor fails when plan missing or inactive`() =
        runBlocking {
            val repo =
                FakeRepo(fixedNow).apply {
    ...
    @Test
    fun `createInvoiceFor succeeds with payload under limit`() =
        runBlocking {
            val repo =
                FakeRepo(fixedNow).apply {
    ...
    @Test
    fun `applySuccessfulPayment activates subscription and is idempotent`() =
        runBlocking {
            val repo = FakeRepo(fixedNow)
            val stars = FakeStarsGateway()
            val service = createService(repo, stars)
    ...
    @Test
    fun `getMySubscription returns stored value or null`() =
        runBlocking {
            val repo = FakeRepo(fixedNow)
            val stored =
                UserSubscription(
    ...
    @Test
    fun `applySuccessfulPayment rejects negative amount`() =
        runBlocking {
            val repo = FakeRepo(fixedNow)
            val service = createService(repo, FakeStarsGateway())
    ...
    @Test
    fun `applySuccessfulPayment without provider id uses deterministic charge id`() =
        runBlocking {
            val repo = FakeRepo(fixedNow)
            val service = createService(repo, FakeStarsGateway())
    ...
    @Test
    fun `duplicate delivery does not upsert subscription twice`() =
        runBlocking {
            val repo = FakeRepo(fixedNow)
            val service = createService(repo, FakeStarsGateway())
    ...
    private class FakeRepo(
        private val defaultStartedAt: Instant,
    ) : BillingRepository {
...
private fun deterministicChargeId(
    userId: Long,
    tier: Tier,
    payload: String?,
): String {
