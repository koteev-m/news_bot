...
data class JwtKeys(val primary: String, val secondary: String?)

fun ApplicationEnvironment.jwtKeys(): JwtKeys {
    val securityConfig = config.config("security")
    val primary = securityConfig.propertyOrNull("jwtSecretPrimary")?.getString()?.trim()?.takeIf { it.isNotEmpty() }
        ?: securityConfig.propertyOrNull("jwtSecret")?.getString()?.trim()?.takeIf { it.isNotEmpty() }
        ?: throw IllegalArgumentException("security.jwtSecretPrimary must not be blank")

    val secondary = securityConfig.propertyOrNull("jwtSecretSecondary")?.getString()?.trim()?.takeIf { it.isNotEmpty() }

    return JwtKeys(primary, secondary)
}

fun verifyWithAny(config: JwtConfig, keys: JwtKeys): JWTVerifier {
...
private fun createVerifier(config: JwtConfig, secret: String): JWTVerifier =
