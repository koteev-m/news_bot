...
data class StartReviewReq(val dueAtIso: String)
data class DecideReq(val itemId: Long, val keep: Boolean)
data class PamReq(val reason: String, val roles: List<String>, val ttlMinutes: Long)
data class ApprovePamReq(val sessionId: Long, val approverId: Long)
data class RevokePamReq(val sessionId: Long, val by: Long?)

fun Route.accessRoutes(reviewSvc: AccessReviewService, sodSvc: SodService, pamSvc: PamService) {
    route("/api/access") {
        post("/review/start") {
