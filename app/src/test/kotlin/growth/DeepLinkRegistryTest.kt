...
class DeepLinkRegistryTest : FunSpec({
    ...
    test("accepts valid start payload") {
        ...
        val result = registry.parse(parametersOf("start" to payload))
    ...
    test("rejects mixed start and startapp") {
        ...
        registry.parse(parametersOf("start" to payload, "startapp" to payload)).shouldBeNull()
    }

    test("enforces length limits") {
        val longPayload = Base64.getUrlEncoder().withoutPadding().encodeToString("a".repeat(600).toByteArray())
        registry.parse(parametersOf("startapp" to longPayload)).shouldBeNull()
    }

    test("rejects non base64url input") {
        registry.parse(parametersOf("start" to "hello!@")) shouldBe null
    }

    test("decoding errors are handled") {
        shouldNotThrowAny {
            registry.parse(parametersOf("start" to "==="))
        }
    }
})
