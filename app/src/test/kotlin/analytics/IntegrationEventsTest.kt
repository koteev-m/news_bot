...
class IntegrationEventsTest {
    @Test
    fun `critical flows emit analytics events`() =
        testApplication {
            ...
            val badResponse =
                client.post("/api/auth/telegram/verify") {
            ...
            val firstWebhook =
                client.post("/telegram/webhook/test") {
                    header(HttpHeaders.ContentType, ContentType.Application.Json.toString())
                    setBody(updateJson)
                }
            ...
            assertEquals(
                listOf("miniapp_auth", "cta_click", "stars_payment_succeeded", "stars_payment_duplicate"),
                types,
            )
    ...
    private class StubBillingService : BillingServiceWithOutcome {
        ...
        override suspend fun listPlans(): Result<List<BillingPlan>> = throw UnsupportedOperationException("not used")

        override suspend fun createInvoiceFor(
            userId: Long,
            tier: Tier,
        ): Result<String> = throw UnsupportedOperationException("not used")
...
private class RecordingReferralsPort : ReferralsPort {
    override suspend fun create(
        ownerUserId: Long,
        code: String,
    ): ReferralCode = ReferralCode(code, ownerUserId)
    ...
    override suspend fun attachUser(
        code: String,
        tgUserId: Long,
    ) { }
}
