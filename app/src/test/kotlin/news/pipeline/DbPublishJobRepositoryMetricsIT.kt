...
class DbPublishJobRepositoryMetricsIT {
    @Test
    fun `metrics counters reflect updated rows`() = runBlocking {
    ...
    private data class JobIdentity(val jobId: UUID, val clusterId: UUID)

    private class RecordingNewsMetrics : NewsMetricsPort {
        private val publishJobStatus = mutableListOf<Pair<PublishJobStatus, Int>>()

        override fun incPublish(type: news.metrics.NewsPublishType, result: news.metrics.NewsPublishResult) {}
        ...
        fun totalFor(status: PublishJobStatus): Int =
            publishJobStatus.filter { it.first == status }.sumOf { it.second }
    }
}
