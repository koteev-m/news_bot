...
class SupportRoutesTest {
    @Test
    fun `GET FAQ returns empty list`() =
        testApplication {
    ...
    fun `feedback is rate limited`() =
        testApplication {
            ...
            val first =
                client.post("/api/support/feedback") {
    ...
    @Test
    fun `feedback accepted triggers analytics`() =
        testApplication {
            val repo = FakeSupportRepository()
            val analytics = RecordingAnalytics()
            val limiter = RateLimiter(RateLimitConfig(5, 5), Clock.systemUTC())
    ...
    private class FakeSupportRepository : SupportRepository() {
        ...
        override suspend fun listFaq(localeValue: String): List<FaqItem> =
            lock.withLock {
                storedFaq[localeValue] ?: emptyList()
            }

        override suspend fun createTicket(ticket: SupportTicket): Long =
            lock.withLock {
                val id = nextId++
