...
class RedirectRoutesTest {
    @Test
    fun `redirect returns deep link and tracks analytics`() =
        testApplication {
    ...
    @Test
    fun `redirect captures referral and utm params`() =
        testApplication {
            val analytics = RecordingAnalytics()
            val referrals = RecordingReferrals()
            val store = InMemoryDeepLinkStore()
            ...
            val response =
                client.get(
                    "/go/promo?ref=R7G5K2&utm_source=channel&utm_medium=cta&utm_campaign=october&utm_extra=${"x".repeat(
                        100,
                    )}",
                )
            assertEquals(302, response.status.value)
