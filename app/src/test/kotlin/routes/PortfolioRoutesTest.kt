...
class PortfolioRoutesTest {
    ...
    @Test
    fun `GET portfolio returns portfolios for authenticated user`() =
        runBlocking {
            val deps = FakeDeps()
            deps.records +=
                listOf(
    ...
    @Test
    fun `POST portfolio creates portfolio`() =
        runBlocking {
            val deps = FakeDeps()
    ...
    @Test
    fun `POST portfolio normalizes base currency`() =
        runBlocking {
    ...
    @Test
    fun `POST portfolio with short name returns 400`() =
        runBlocking {
            val deps = FakeDeps()
    ...
    @Test
    fun `POST portfolio with duplicate name returns 409`() =
        runBlocking {
    ...
    @Test
    fun `requests without JWT return 401`() =
        runBlocking {
            val deps = FakeDeps()
