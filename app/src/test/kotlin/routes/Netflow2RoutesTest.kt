...
class Netflow2RoutesTest {
    ...
    @Test
    fun `validates required parameters`() =
        testApplication {
            val loader = io.mockk.mockk<Netflow2Loader>()
            ...
            val response =
                client.post("/admin/netflow2/load?sec=&from=2024-01-01") {
    ...
    @Test
    fun `rejects ticker with whitespace`() =
        testApplication {
            val loader = io.mockk.mockk<Netflow2Loader>()
            val from = LocalDate.of(2024, 1, 1)
            val till = LocalDate.of(2024, 1, 2)
            ...
            val response =
                client.post("/admin/netflow2/load?sec=S%20BER&from=2024-01-01&till=2024-01-02") {
                    headers { append(INTERNAL_TOKEN_HEADER, "token") }
                }
    ...
    @Test
    fun `returns not found for unknown sec`() =
        testApplication {
            val loader = io.mockk.mockk<Netflow2Loader>()
            val from = LocalDate.of(2024, 1, 1)
            val till = LocalDate.of(2024, 1, 2)
            ...
            val response =
                client.post("/admin/netflow2/load?sec=SBERX&from=2024-01-01&till=2024-01-02") {
                    headers { append(INTERNAL_TOKEN_HEADER, "token") }
                }
