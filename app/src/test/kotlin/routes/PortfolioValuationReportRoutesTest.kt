...
class PortfolioValuationReportRoutesTest {
    ...
    @Test
    fun `revalue without JWT returns 401`() =
        testApplication {
            val deps = FakeDeps()
    ...
    @Test
    fun `report without JWT returns 401`() =
        testApplication {
            val deps = FakeDeps()
            application { configureTestApp(deps.toServices()) }
    ...
    @Test
    fun `invalid portfolio id on revalue returns 400`() =
        testApplication {
            val deps = FakeDeps()
            application { configureTestApp(deps.toServices()) }
    ...
    @Test
    fun `missing date parameter returns 400`() =
        testApplication {
            val deps = FakeDeps()
            application { configureTestApp(deps.toServices()) }
    ...
    @Test
    fun `invalid date format on revalue returns 400`() =
        testApplication {
            val deps = FakeDeps()
            application { configureTestApp(deps.toServices()) }
    ...
    @Test
    fun `invalid portfolio id on report returns 400`() =
        testApplication {
            val deps = FakeDeps()
            application { configureTestApp(deps.toServices()) }
    ...
    @Test
    fun `missing from parameter on report returns 400`() =
        testApplication {
            val deps = FakeDeps()
            application { configureTestApp(deps.toServices()) }
    ...
    @Test
    fun `invalid to parameter on report returns 400`() =
        testApplication {
            val deps = FakeDeps()
            application { configureTestApp(deps.toServices()) }
    ...
    @Test
    fun `range where to before from returns 400`() =
        testApplication {
            val deps = FakeDeps()
            application { configureTestApp(deps.toServices()) }
    ...
    @Test
    fun `revalue returns valuation response`() =
        testApplication {
            val deps = FakeDeps()
            ...
            deps.valuationStorage.upsertResult =
    ...
    @Test
    fun `metrics report returns json`() =
        testApplication {
            val deps = FakeDeps()
            val portfolioId = UUID.randomUUID()
            deps.metricsStorage.valuationsResult =
                DomainResult.success(
                    listOf(
    ...
    @Test
    fun `metrics report monthly clamps period end to last valuation`() =
        testApplication {
            val deps = FakeDeps()
            val portfolioId = UUID.randomUUID()
            deps.metricsStorage.valuationsResult =
    ...
    @Test
    fun `metrics report weekly clamps period end to last valuation`() =
        testApplication {
            val deps = FakeDeps()
            val portfolioId = UUID.randomUUID()
            deps.metricsStorage.valuationsResult =
            ...
            val token = issueToken("119")
            val response =
                get(
                    path = "/api/portfolio/$portfolioId/report?period=weekly&base=RUB",
                    headers = authHeader(token),
                )
            assertEquals(HttpStatusCode.OK, response.status)
            val payload = json.decodeFromString<PortfolioMetricsReportResponse>(response.body)
            assertEquals("weekly", payload.period)
            assertEquals(1, payload.series.size)
            assertEquals("2024-01-02", payload.series.first().periodEnd)
        }

    @Test
    fun `metrics report returns csv`() =
        testApplication {
            val deps = FakeDeps()
            val portfolioId = UUID.randomUUID()
            deps.metricsStorage.valuationsResult =
    ...
    @Test
    fun `metrics report returns 502 on fx errors`() =
        testApplication {
            val deps = FakeDeps()
            val portfolioId = UUID.randomUUID()
            deps.metricsStorage.valuationsResult =
    ...
    @Test
    fun `metrics report rejects invalid period`() =
        testApplication {
            val deps = FakeDeps()
            application { configureTestApp(deps.toServices()) }
    ...
    @Test
    fun `revalue returns 404 when portfolio missing`() =
        testApplication {
            val deps = FakeDeps()
            deps.valuationStorage.listPositionsResult =
    ...
    @Test
    fun `report returns 404 when portfolio missing`() =
        testApplication {
            val deps = FakeDeps()
            deps.reportStorage.valuationMethodResult =
                DomainResult.failure(
                    PortfolioException(PortfolioError.NotFound("portfolio missing")),
                )
            application { configureTestApp(deps.toServices()) }
    ...
    @Test
    fun `revalue unexpected error returns 500`() =
        testApplication {
            val deps = FakeDeps()
    ...
    @Test
    fun `report unexpected error returns 500`() =
        testApplication {
            val deps = FakeDeps()
    ...
    private class SimpleTestApplication {
        ...
        fun post(
            path: String,
            headers: Map<String, String> = emptyMap(),
        ): SimpleHttpResponse = request("POST", path, headers)
    ...
    private class FakeDeps {
        ...
        private val moexProvider =
            object : MoexPriceProvider {
                override suspend fun closePrice(
                    instrumentId: Long,
                    on: LocalDate,
                ) = DomainResult.success<Money?>(null)
        ...
        private val coingeckoProvider =
            object : CoingeckoPriceProvider {
                override suspend fun closePrice(
                    instrumentId: Long,
                    on: LocalDate,
                ) = DomainResult.success<Money?>(null)
