...
class PostViewsRoutesTest {
    ...
    @Test
    fun `enabled route returns views and increments metrics`() =
        testApplication {
            ...
            val response =
                client.get("/internal/post_views/sync?channel=news&ids=1,2") {
    ...
    @Test
    fun `disabled route returns not implemented`() =
        testApplication {
            application {
                install(ContentNegotiation) { json() }
                routing {
            ...
            val response =
                client.get("/internal/post_views/sync?channel=news&ids=1") {
                    headers.append(INTERNAL_TOKEN_HEADER, "secret")
                }
    ...
    @Test
    fun `invalid ids return bad request`() =
        testApplication {
            val registry = SimpleMeterRegistry()
            ...
            val response =
                client.get("/internal/post_views/sync?channel=news&ids=abc") {
                    headers.append(INTERNAL_TOKEN_HEADER, "secret")
                }
    ...
    @Test
    fun `missing internal token returns forbidden`() =
        testApplication {
            val registry = SimpleMeterRegistry()
            val service = PostViewsService(StaticViewsClient(emptyMap()), registry)
    ...
    @Test
    fun `zero or negative ids return bad request`() =
        testApplication {
            val registry = SimpleMeterRegistry()
            val service = PostViewsService(StaticViewsClient(emptyMap()), registry)
            ...
            val response =
                client.get("/internal/post_views/sync?channel=news&ids=0,-1") {
                    headers.append(INTERNAL_TOKEN_HEADER, "secret")
                }
            assertEquals(HttpStatusCode.BadRequest, response.status)
        }
...
private class StaticViewsClient(
    private val response: Map<Int, Long>,
) : ChannelViewsClient {
    override suspend fun getViews(
        channel: String,
        ids: List<Int>,
        increment: Boolean,
    ): Map<Int, Long> = response
}
