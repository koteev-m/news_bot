...
class Netflow2LoaderTest {
    @Test
    fun `splits range into windows and upserts sequentially`() =
        runTest {
    ...
    @Test
    fun `records metrics for successful load`() =
        runTest {
    ...
    fun `rejects ticker with whitespace`() =
        runTest {
            ...
            val ex =
                assertFailsWith<Netflow2LoaderError.ValidationError> {
    ...
    @Test
    fun `rejects ticker with unsupported characters`() =
        runTest {
            val metrics = Netflow2Metrics(SimpleMeterRegistry())
            val client = io.mockk.mockk<Netflow2Client>()
            val repository = io.mockk.mockk<Netflow2Repository>()
    ...
    @Test
    fun `propagates assertion error from client`() =
        runTest {
            val metrics = Netflow2Metrics(SimpleMeterRegistry())
            val client = io.mockk.mockk<Netflow2Client>()
            val repository = io.mockk.mockk<Netflow2Repository>()
