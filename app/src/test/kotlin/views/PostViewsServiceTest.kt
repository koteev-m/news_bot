...
class PostViewsServiceTest {
    @Test
    fun `sync increments post views metric per post`() =
        runTest {
            val registry = SimpleMeterRegistry()
            val client =
                CapturingViewsClient(
                    listOf(
    ...
    @Test
    fun `sync ignores view count decreases`() =
        runTest {
            val registry = SimpleMeterRegistry()
            val client =
                CapturingViewsClient(
...
private class CapturingViewsClient(
    private val responses: List<Map<Int, Long>>,
) : ChannelViewsClient {
