...
class WebhookQueueTest {
    @Test
    fun `capacity overflow drops updates and counts processed`() =
        runTest {
    ...
    @Test
    fun `multiple workers consume updates`() =
        runTest {
            val registry = SimpleMeterRegistry()
            val metrics = WebhookMetrics.create(registry)
            val processed = AtomicInteger(0)
    ...
    @Test
    fun `stop timer once on cancellation`() =
        runTest {
            val registry = SimpleMeterRegistry()
            val metrics = WebhookMetrics.create(registry)
