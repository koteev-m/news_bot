...
class StarsWebhookHandlerTest {
    @Test
    fun `successful payment invokes billing service`() =
        testApplication {
    ...
    @Test
    fun `payload user mismatch prevents processing`() =
        testApplication {
            val service = RecordingBillingService()
            application { installParsedRoute("/test/webhook", service) }
    ...
    @Test
    fun `handleParsed uses provided update without re-reading body`() =
        testApplication {
            val service = RecordingBillingService()
    ...
    @Test
    fun `provider payment id null is passed to billing`() =
        testApplication {
            val service = RecordingBillingService()
            application { installParsedRoute("/test/webhook", service) }
    ...
    private fun Application.installParsedRoute(
        path: String,
    ...
    private fun successfulPaymentJson(
        ...
        val providerField =
            providerChargeId?.let { "\"provider_payment_charge_id\":\"$it\"" }
