...
class Netflow2LoaderIT {
    @Test
    fun `loads two windows into postgres idempotently`() =
        runBlocking {
            TestDb.withMigratedDatabase { _ ->
                ...
                io.mockk.coEvery { client.fetchWindow("SBER", windows[0]) } returnsMany
                    listOf(
                        Result.success(firstWindowRows),
                        Result.success(updatedFirstWindowRows),
                    )
                io.mockk.coEvery { client.fetchWindow("SBER", windows[1]) } returnsMany
                    listOf(
