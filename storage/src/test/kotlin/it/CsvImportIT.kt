...
class CsvImportIT {
    @Test
    fun `csv import reports inserted duplicates and failed rows`() =
        runBlocking {
            TestDb.withMigratedDatabase { _ ->
                ...
                val instrument =
                    instrumentRepository.createInstrument(
                        NewInstrument(
                            clazz = "EQUITY",
                            exchange = "MOEX",
                            board = "TQBR",
                ...
                TestDb.tx {
                    PositionsTable
                        ...
                        .map { row ->
                            ...
                            val averageCurrency =
                                row[PositionsTable.avgPriceCcy]
                                    ?: instrument?.currency
                                    ?: fallbackCurrency
            ...
            try {
                val result =
                    PositionsTable
                        .selectAll()
                        .where {
                            (PositionsTable.portfolioId eq portfolioId) and
                                (PositionsTable.instrumentId eq instrumentId)
                        }.limit(1)
                        .singleOrNull()
                        ?.let { row ->
...
private class DatabaseInstrumentResolver(
    ...
    override suspend fun findByAlias(
        ...
        try {
            val instrument =
                instrumentRepository.findAlias(alias, source)?.let {
